...
public class CustomAuthenticationFilter extends AbstractAuthenticationProcessingFilter {
    ...
    private final transient byte[] keyHMAC =new byte[12311724];
...
//set the expected cookie itself
    ...
    protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain, Authentication authentication) throws IOException {
        ...
        byte[] randomFgp = new byte[50];  // token sideJacking tizio random che ti ruba er token
        secureRandom.nextBytes(randomFgp);
        String userFingerprint = DatatypeConverter.printHexBinary(randomFgp);


        String fingerprintCookie =  userFingerprint+"HttpOnly";
